...
numpy.random.seed(7)
# load the dataset
dataframe = pd.read_csv('oil_production.csv', usecols=[1], engine='python')
...
model.add(GRU(64, input_shape=(1, look_back),return_sequences=True))
model.add(Dropout(0.3))
model.add(GRU(64, input_shape=(1, look_back)))
model.add(Dense(1))
model.summary()
model.compile(loss='mean_squared_error', optimizer='adam')
model.fit(trainX, trainY, epochs=40 , batch_size=8, verbose=1,validation_data=(testX, testY))
# make predictions
...
#-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*|
plt.plot(testVaild,label='original', color='blue')
#-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*|
